...
add_image_size( 'home_blog', 1400, 1298 );


add_action( 'rest_api_init', 'register_featuredimg' );
function register_featuredimg() {
    register_rest_field( 'post',
        'featuredimg',
        array(
            'get_callback'    => 'featuredimg_get',
...
function featuredimg_get( $object, $field_name, $request ) {
  $result = false;
  $thumb_id = get_post_thumbnail_id( $object[ 'id' ] );
  $thumb_url = wp_get_attachment_image_src($thumb_id,'home_blog', false);
  if($thumb_url){
    $result = $thumb_url[0];
  }
  return  $result;
...
//META TAGS FIELDS
add_action( 'rest_api_init', 'register_yoast_title' );
function register_yoast_title() {
    register_rest_field( 'post',
        'yoastTitle',
        array(
            'get_callback'    => 'yoast_title_get',
            'update_callback' => null,
            'schema'          => null,
        )
    );
...
function yoast_title_get( $object, $field_name, $request ) {
  $id = $object['id'];
  return  get_post_meta($id, '_yoast_wpseo_title', true);
}

//META TAGS FIELDS
add_action( 'rest_api_init', 'register_yoast_description' );
function register_yoast_description() {
    register_rest_field( 'post',
        'yoastDescription',
        array(
            'get_callback'    => 'yoast_description_get',
            'update_callback' => null,
            'schema'          => null,
        )
    );
...
function yoast_description_get( $object, $field_name, $request ) {
  $id = $object['id'];
  return  get_post_meta($id, '_yoast_wpseo_metadesc', true);
}

//META TAGS FIELDS
add_action( 'rest_api_init', 'register_yoast_keywords' );
function register_yoast_keywords() {
    register_rest_field( 'post',
        'yoastKeywords',
        array(
            'get_callback'    => 'yoast_keywords_get',
            'update_callback' => null,
            'schema'          => null,
        )
    );
...
function yoast_keywords_get( $object, $field_name, $request ) {
  $id = $object['id'];
